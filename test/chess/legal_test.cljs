(ns chess.legal-test
  (:require [cljs.test :refer-macros [deftest is testing are]]
            [chess.legal :refer [pawn-two-square-move-from-initial-rank?
                                 is-legal-pawn-move?
...
(deftest is-legal-test
  (testing "Testing legality of various moves"
    (is (true? (is-legal? white-pawn 4 4 test-board en-passant-target))) ; Legal pawn move from e2 to e4
    (is (true? (is-legal? black-pawn 4 2 test-board en-passant-target))) ; Legal pawn move from e7 to e6
    (is (false? (is-legal? black-pawn 3 2 test-board en-passant-target))) ; Illegal diagonal pawn move from e7 to d6
    (is (false? (is-legal? white-king 4 6 test-board en-passant-target))) ; Illegal king move from e1 to e2 (blocked)
    (is (false? (is-legal? black-king 5 0 test-board en-passant-target))))) ; Illegal king move from e8 to f8 (blocked)

;; TODO finishing fixing up these broken tests, and then review all tests
#_(deftest can-be-captured?-test
    (testing "Testing if a piece can be captured"
      (is (true? (can-be-captured? 'w test-board en-passant-target 0 6))) ; Pawn on a7 can be captured by b6
      (is (false? (can-be-captured? 'b test-board en-passant-target 7 1))))) ; Pawn on h2 cannot be captured

#_(deftest in-check?-test
    (testing "Testing if a king is in check"
      (is (false? (in-check? 'w test-board en-passant-target))) ; White king is not in check initially
      (is (false? (in-check? 'b test-board en-passant-target))))) ; Black king is not in check initially
...
(deftest first-capture-or-rand-test
  (testing "Testing that first-capture-or-rand selects a capture if available"
    (let [moves [{:piece white-pawn :end-x 4 :end-y 5 :capture 'p :can-be-captured nil}]
      ...
      (is (= (first-capture-or-rand captures moves) {:piece white-pawn :end-x 4 :end-y 5 :capture 'p :can-be-captured nil})))))

#_(deftest get-all-legal-moves-not-in-check-test
    (testing "Fetching legal moves not resulting in check"
      (is (some #(= % {:piece white-pawn :end-x 4 :end-y 5 :capture nil :can-be-captured false}) (get-all-legal-moves-not-in-check 'w test-board en-passant-target)))
      (is (empty? (filter #(= % {:piece black-king :end-x 4 :end-y 7 :capture nil :can-be-captured false}) (get-all-legal-moves-not-in-check 'b test-board en-passant-target))))))

(deftest get-regular-move-test
  (testing "Getting a regular move"
    (is (not (nil? (get-regular-move 'w test-board en-passant-target))))
    (is (not (nil? (get-regular-move 'b test-board en-passant-target))))))

(deftest get-openings-table-move-or-castle-test
  (testing "Testing openings table move or castle functionality"
    (is (nil? (get-openings-table-move-or-castle "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1" 'w test-board))))) ; Test with a common starting position FEN

(deftest any-possible-moves?-test
  (testing "Testing if any possible moves are available"
    (is (true? (any-possible-moves? 'w test-board en-passant-target))) ; White has possible moves
    (is (true? (any-possible-moves? 'b test-board en-passant-target))))) ; Black has possible moves

(deftest can-castle-queenside?-test
  (testing "Testing if queenside castling is possible"
    (is (false? (can-castle-queenside? 'w test-board castling in-check?))) ; White cannot castle queenside initially
    (is (false? (can-castle-queenside? 'b test-board castling in-check?))))) ; Black cannot castle queenside initially

(deftest can-castle-kingside?-test
  (testing "Testing if kingside castling is possible"
    (is (false? (can-castle-kingside? 'w test-board castling in-check?))) ; White cannot castle kingside initially
    (is (false? (can-castle-kingside? 'b test-board castling in-check?))))) ; Black cannot castle kingside initially
